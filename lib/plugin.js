    ...
    schema.statics.getFriends = function(model, { conditions = {}, fields, options } = {}) {
      ...
      const getLocals = Model.findOne({ _id: model }, { [pathName]: 1 })
        .then((doc) => {
          ...
          return doc[pathName].reduce((o, friend) => {
      ...
      const wrapResultsWithStatus = (friends, locals) => {
        for (let i = 0; i < friends.length; i++) {
          let friendship = locals[friends[i]._id];

          if (!friendship) {
            continue;
          }

          friendship.friend = friends[i];
          friends[i] = friendship;
        }
      };
      ...
      return Promise.all([
        ...
        .then(([friends, locals]) => {
          wrapResultsWithStatus(friends, locals);
          return friends;
        });
    };
